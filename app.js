(() => {
  ...
  function renderCommit(inBugfix = false) {
    ...
    const ownershipKnown = task.git?.taskChangeOwnershipKnown === true;
    const stagedOutsideTask = [...mixedEntries, ...foreignEntries].filter((item) => {
    ...
    return `<section class="section">${callout(`<strong>${inBugfix ? "Bug 修复的最后一道 Git 写入门" : "最后一道 Git 写入门"}。</strong> Commit 只作用于当前 Worktree。配套验收 HTML 位于配置的 docsRoot：<code>${escapeHTML(health?.paths?.docs || "")}</code>；若它在仓库外则不属于此 Git 提交。不会 Push 或 Merge。`, "warning")}</section>
      ${!ownershipKnown ? callout("<strong>该任务缺少可靠的文件归属快照。</strong> 历史 Agent 文件清单不会再被当作 Stage 权限；请在控制台外手动按文件处理，或重新执行任务建立归属。", "warning") : ""}
      ${stagedOutsideTask.length ? callout(`<strong>暂存区已有 ${stagedOutsideTask.length} 项非当前任务内容。</strong> 为保证一键 Stage 后只有当前任务修改，按钮已锁定；请先在控制台外处理现有暂存区。`, "warning") : ""}
