...
- **官方 Lark CLI（`lark_cli`）**：支持 `feishu.cn`、`larksuite.com` 和 `larkoffice.com` 的 `/docx/`、`/wiki/` 文档链接。服务使用现有飞书用户授权，执行固定的 `docs +fetch`，并尝试用 `sheets +csv-get` 读取可解析的内嵌表格。服务端对 CLI 返回的原文、章节和表格完整性元数据做确定性校验并保存，不依赖后台 Codex 二次复核。
...
选择 Lark CLI 时，创建任务后自动读取并由服务端校验原文、章节和表格完整性元数据，材料就绪后同样需要点击“使用材料”才会开始讨论。无法读取的图片、画板、Base、表格或引用保留在未读列表；是否阻断由当前附件策略决定。读取阶段只使用飞书用户授权；页面显示的后台连接用于后续讨论，桌面聊天选择不会改变它。详见 [连接配置与读取环境](docs/codex-connections.md)。
